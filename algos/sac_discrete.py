...
class SACDiscrete(RlAlgorithm):
    ...
    def optim_initialize(self, rank=0):
        ...
        if self.fixed_alpha is None:
            self.target_entropy = -np.log((1.0 / self.agent.env_spaces.action.n)) * 0.98
            self._log_alpha = torch.zeros(1, requires_grad=True)
            self._alpha = self._log_alpha.exp()
            self.alpha_optimizer = self.OptimCls((self._log_alpha,), lr=self.learning_rate, **self.optim_kwargs)
        else:
    ...
    def loss(self, samples):
        ...
        with torch.no_grad():
            target_action, target_action_probs, target_log_pi, _ = self.agent.pi(*target_inputs)
