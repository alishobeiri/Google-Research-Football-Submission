...
class MoE(nn.Module):
    ...
    def forward(self, observation, prev_action, prev_reward, loss_coef=1e-2):
        ...
        gates, load = self.noisy_top_k_gating(observation, train)
        importance = gates.sum(0)
        loss = self.cv_squared(importance) + self.cv_squared(load)
        loss *= loss_coef

        dispatcher = SparseDispatcher(self.num_experts, gates)
        expert_inputs = dispatcher.dispatch(observation)
    ...
    def loss(self, observation, prev_action, prev_reward, loss_coef=1e-2):
        train = self.training
        lead_dim, T, B, obs_shape = infer_leading_dims(observation, 1)
        observation = observation.view(T * B, *obs_shape)
